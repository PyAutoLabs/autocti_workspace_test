...
for norm in norm_list:
    ...
    pre_cti_image_list.append(
        ac.Array2D.from_fits(
            file_path=path.join(dataset_path, "pre_cti_data.fits"),
            hdu=0,
            pixel_scales=0.1,
        )
    )
...
"""
serial_trap_0 = ac.TrapInstantCapture(density=0.1, release_timescale=1.0)
...
for pre_cti_image in pre_cti_image_list:
    ...
    clocker = ac.Clocker2D(
        ...
        serial_prune_n_electrons=1.0e-18,
    ...
    """
    clocker = ac.Clocker2D(
        serial_express=serial_express, serial_roe=serial_roe, serial_prune_frequency=0
    )
