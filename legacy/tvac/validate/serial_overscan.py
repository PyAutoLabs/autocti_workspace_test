...
for extension in extension_list:
    ...
    which in this example uses the serial overscan.
    ...
    """
    Stash the serial overscan to ensure all dataseta are identical at the end.
    """
    serial_overscan_list.append(layout_2d.serial_overscan)

    """
    Extract an array only containing the serial overscan.
    
    This array should not contain any signal, and therefore have values near the bias level of ~2600e-.
    """
    array_2d_list = layout_2d.extract.serial_overscan.array_2d_list_from(
        array=image_ci, pixels=(0, 29)
    )

    print(f"Serial Overscan Min value (Expect ~2600e-) = {np.min(array_2d_list[0])}")

    """
    Extract an array containing the serial overscan and the first column of the serial FPR.
    
    This array should contain signal, and therefore have maximum values near ~3600e-
    """
    array_2d_list = layout_2d.extract.serial_overscan.array_2d_list_from(
        array=image_ci, pixels=(-1, 29)
    )

    print(f"Serial Overscan w/ FPR Max (Expect >3000e-) = {np.max(array_2d_list[0])}")
    print(f"CI Overscan w/ FPR Min (Expect ~2600e-) = {np.min(array_2d_list[0])}")


print(
    "\n Check that Serial Overscan For Every Dataset Are Same for Every CCD / Quad ID:"
)

for serial_overscan in serial_overscan_list:

    print(f"Serial Overscan: {serial_overscan}")
