...
for extension in extension_list:
    ...
    which in this example uses the serial prescan.
    ...
    """
    Stash the serial prescan to ensure all dataseta are identical at the end.
    """
    serial_prescan_list.append(layout_2d.serial_prescan)

    """
    Extract an array only containing the serial prescan.
    
    This array should not contain any signal, and therefore have values near the bias level of ~2600e-.
    """
    array_2d_list = layout_2d.extract.serial_prescan.array_2d_list_from(
        array=image_ci, pixels=(0, 51)
    )

    print(f"Serial Prescan Min value (Expect ~2600e-) = {np.min(array_2d_list[0])}")

    """
    Extract an array containing the serial prescan and the first column of the serial FPR.
    
    This array should contain signal, and therefore have maximum values near ~3600e-
    """
    array_2d_list = layout_2d.extract.serial_prescan.array_2d_list_from(
        array=image_ci, pixels=(0, 52)
    )

    print(f"Serial Prescan w/ FPR Max (Expect >3000e-) = {np.max(array_2d_list[0])}")
    print(f"CI Prescan w/ FPR Min (Expect ~2600e-) = {np.min(array_2d_list[0])}")


print(
    "\n Check that Serial Prescan For Every Dataset Are Same for Every CCD / Quad ID:"
)

for serial_prescan in serial_prescan_list:

    print(f"Serial Prescan: {serial_prescan}")
